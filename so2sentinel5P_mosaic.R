######################################################################################
# This is an R script to create a raster map from L2 SO2 SENTINEL 5P.                #   
...
# Created by Alberth Nahas on 2021-12-12 01:00 pm WIB.                               #
...
for (i in seq_along(fn)) {
  nc <- nc_open(fn[i])
  tcso2 <- ncvar_get(nc, "PRODUCT/sulfurdioxide_total_vertical_column")
  ...
  tcso2 <- tcso2 * mfactor$value * 1e-15 # expressed in 10^15 molecules per cm^2
  lat <- ncvar_get(nc, "PRODUCT/latitude")
  lon <- ncvar_get(nc, "PRODUCT/longitude")
  # concatenate the new data to the global data frame
  so2df <- rbind(so2df, data.frame(lat=as.vector(lat), 
                                   lon=as.vector(lon), 
                                   tcso2=as.vector(tcso2)))
  # close file
  nc_close(nc)
}

### CREATE A .csv FILE TO BUILD THE RASTER FILE ###
write.csv(so2df, file = csvname, row.names = FALSE, quote = TRUE, na = "NA")
...
rst <- rasterize(pts, rst01, pts$tcso2, fun = max) # max is used instead of mean
...
writeRaster(ncdf, 
            ...
            longname = "Total Vertical Column SO2 Sentinel-5P",
            xname = "longitude",
            yname = "latitude", 
